...
def validate_bank_carveout_rules(db_path: str = 'nifty100.db') -> Dict[str, Any]:
    ...
    if 'broad_sector' in sec.columns:
        fin_companies = set(sec[sec['broad_sector'].str.contains('Financial|Bank|Finance|Insurance', case=False, na=False)]['company_id'])
    ...
    print(f"Financial ROCE non-null count (Expected: 0)     : {fin_roce_non_null} ({'PASS' if fin_roce_non_null==0 else 'FAIL'})")
    print(f"Financial High Leverage flag count (Expected: 0): {fin_high_lev_active} ({'PASS' if fin_high_lev_active==0 else 'FAIL'})")
    print(f"Non-Financial D/E > 5.0 correctly flagged       : {non_fin_flagged}/{non_fin_total_over_5} ({'PASS' if non_fin_flagged==non_fin_total_over_5 else 'FAIL'})")
