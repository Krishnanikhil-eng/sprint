...
def export_capital_allocation(db_path: str = 'nifty100.db', output_path: str = 'output/capital_allocation.csv') -> str:
    conn = sqlite3.connect(db_path)
    
    cf_df = pd.read_sql_query("SELECT company_id, year, operating_activity, investing_activity, financing_activity FROM cashflow", conn)
    pnl_df = pd.read_sql_query("SELECT company_id, year, net_profit FROM profitandloss", conn)
    ...
    for _, row in merged.iterrows():
        ...
        cfo_sign, cfi_sign, cff_sign, label = classify_capital_allocation(cfo, cfi, cff, cfo_pat_ratio)
