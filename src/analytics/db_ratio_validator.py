...
def validate_financial_ratios_db(db_path: str = "nifty100.db") -> Dict[str, Any]:
    ...
    print(
        "               DATABASE RATIOS HEALTH & COMPLETENESS AUDIT                          "
    )
    print(
        "==================================================================================="
    )
    print(
        f"Row count                      : {row_count} (>= 1,100: {'PASS' if row_count>=1100 else 'FAIL'})"
    )
    print(
        f"Duplicate (company_id, year)   : {dup_count} (== 0: {'PASS' if dup_count==0 else 'FAIL'})"
    )
