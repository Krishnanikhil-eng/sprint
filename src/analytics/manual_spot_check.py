...
def run_manual_spot_checks(db_path: str = 'nifty100.db') -> bool:
    ...
    all_passed = True
    print("===================================================================================")
    print("               SPRINT 2 — 3-COMPANY MANUAL SPOT-CHECK VALIDATION                    ")
    print("===================================================================================")
    ...
    for cid in sample_companies:
        ...
        db_ratio_row = ratios[(ratios['company_id'] == cid) & (ratios['year'] == target_year)]
        ...
        if not pnl_row.empty and not bs_row.empty and not db_ratio_row.empty:
            ...
            print(f"ROE (2024): Manual = {manual_roe:.4f}% | DB = {db_roe:.4f}% | Diff = {diff_roe:.4f}% | Result: {'PASS' if pass_roe else 'FAIL'}")
            if not pass_roe:
                all_passed = False
        else:
        ...
        if not pnl_base.empty and not pnl_end.empty and not db_ratio_row.empty:
            ...
            print(f"Revenue CAGR (5Y): Manual = {manual_cagr:.4f}% | DB = {db_cagr:.4f}% | Diff = {diff_cagr:.4f}% | Result: {'PASS' if pass_cagr else 'FAIL'}")
            if not pass_cagr:
                all_passed = False
        else:
    ...
    print("\n===================================================================================")
    print(f"FINAL MANUAL SPOT CHECK RESULT: {'PASS (All diffs < 0.1%)' if all_passed else 'FAIL'}")
    print("===================================================================================")
    return all_passed
