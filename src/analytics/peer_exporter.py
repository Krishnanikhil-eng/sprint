...
class PeerExporter:
    ...
    def __init__(
        self, peer_engine: Optional[PeerEngine] = None, output_dir: str = "output"
    ):
    ...
    def export_peer_report(self, file_name: str = "peer_comparison.xlsx") -> str:
        ...
        df_peers[existing_cols].to_excel(
            writer, sheet_name="All Peer Percentiles", index=False
        )
        ...
        for grp in top_groups[:2]:
            ...
            if len(grp_companies) > 0:
                top_company = grp_companies.sort_values(
                    by="overall_peer_percentile", ascending=False
                ).iloc[0]
                ...
                vals = [
                    ...
                    float(
                        top_company.get("operating_profit_margin_pct_percentile", 50)
                    ),
