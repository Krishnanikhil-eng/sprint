...
def run_ratio_engine(db_path: str = "nifty100.db") -> pd.DataFrame:
    ...
    pnl_excel = load_excel('data/profitandloss.xlsx').dropna(subset=['year']).drop_duplicates(subset=['company_id', 'year'])
    bs_excel = load_excel('data/balancesheet.xlsx').dropna(subset=['year']).drop_duplicates(subset=['company_id', 'year'])
    cf_excel = load_excel('data/cashflow.xlsx').dropna(subset=['year']).drop_duplicates(subset=['company_id', 'year'])
    
    # Identify Financials sector companies
    fin_companies = set()
    if 'broad_sector' in sec_df.columns:
        fin_companies = set(sec_df[sec_df['broad_sector'].str.contains('Financial|Bank|Finance|Insurance', case=False, na=False)]['company_id'])

    # Merge DB and Excel to construct master statement dataset
    m_pnl = pd.concat([pnl_df, pnl_excel]).drop_duplicates(subset=['company_id', 'year'])
    ...
    master_df = pd.merge(m_pnl, m_bs, on=['company_id', 'year'], how='outer', suffixes=('', '_bs'))
    master_df = pd.merge(master_df, m_cf, on=['company_id', 'year'], how='outer', suffixes=('', '_cf'))
    ...
    for _, row in master_df.iterrows():
        ...
        ebit = (op_profit or 0.0) + (other_income or 0.0) if (op_profit is not None or other_income is not None) else None
        roce = calculate_roce(ebit, equity_cap, reserves, borrowings, is_financials=is_fin)
        ...
        icr, icr_lbl, icr_warn = calculate_interest_coverage(op_profit, other_income, interest)
        ...
        cfo_5y = [company_cfo[cid].get(y) for y in range(yr - 4, yr + 1) if y in company_cfo[cid]]
        pat_5y = [company_pat[cid].get(y) for y in range(yr - 4, yr + 1) if y in company_pat[cid]]
        cfo_score, cfo_score_lbl = calculate_cfo_quality_score(cfo_5y, pat_5y)
        
        # Capital Allocation Classifier
        cfo_pat_r = (cfo / net_profit) if (cfo is not None and net_profit is not None and net_profit != 0) else None
    ...
    print(f"Ratio Engine execution completed. Loaded {len(df_ratios)} rows into financial_ratios table.")
    return df_ratios
