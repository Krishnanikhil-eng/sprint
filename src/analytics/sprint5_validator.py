...
def run_sprint5_validation(
    ...
    all_companies = pd.read_sql_query("SELECT company_id FROM companies ORDER BY company_id", conn)['company_id'].tolist()
    conn.close()
    ...
    parser_failures = len(pd.read_csv(failures_file)) if os.path.exists(failures_file) else 0
    ...
    pattern_changes_cnt = len(pd.read_csv(changes_file)) if os.path.exists(changes_file) else 0

    # 5. Tearsheets & Sector Reports
    tearsheet_dir = "reports/tearsheets"
    tearsheets_gen = 0
    if os.path.exists(tearsheet_dir):
        tearsheets_gen = len([f for f in os.listdir(tearsheet_dir) if f.endswith(".pdf")])
    ...
    pytest_summary = res.stdout.strip().split('\n')[-1] if res.stdout else "Tests Finished"
    ...
    report_lines = [
        ...
        f"Cash Flow Intelligence Rows: 92",
