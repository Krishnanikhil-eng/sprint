...
from src.dashboard.config import LABEL_NA, SUPPORTED_YEARS
...
if ratios_df.empty:
    st.warning(
        f"No data available for year {selected_year}. Please select another year."
    )
    st.stop()
...
with col6:
    st.metric("Debt-Free Companies", debt_free_count, help="Companies with D/E < 0.1")
...
if "composite_quality_score" in ratios_df.columns:
    top_quality = ratios_df.dropna(subset=["composite_quality_score"]).nlargest(
    ...
    if not top_quality.empty:
        for idx, row in top_quality.reset_index().iterrows():
            ...
            st.markdown(
                f"**{idx + 1}. {row['company_name']}** ({sector_name}) — Composite Score: `{score:.2f}`"
            )
    else:
