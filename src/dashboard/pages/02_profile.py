...
with search_col2:
    company_options = ["Select a company..."] + sorted(
        companies_df["company_name"].dropna().unique().tolist()
    )
...
with col2:
    st.markdown(
        f"**NSE Ticker:** {company_data.get('nse_profile', company_data.get('company_id', LABEL_NA))}"
    )
...
with chart_col1:
    ...
    if not pl_data.empty and "sales" in pl_data.columns:
        ...
        pl_sorted["sales"] = pd.to_numeric(pl_sorted["sales"], errors="coerce").fillna(
            0
        )
        pl_sorted["net_profit"] = pd.to_numeric(
            pl_sorted["net_profit"], errors="coerce"
        ).fillna(0)
...
with chart_col2:
    ...
    if not ratios_data.empty:
        ...
        ratios_sorted["return_on_equity_pct"] = pd.to_numeric(
            ratios_sorted["return_on_equity_pct"], errors="coerce"
        )
        ratios_sorted["roce_pct"] = pd.to_numeric(
            ratios_sorted["roce_pct"], errors="coerce"
        )
