...
merged_df = pd.merge(
    ratios_df, companies_df[["company_id", "company_name"]], on="company_id", how="left"
)

if "company_name_y" in merged_df.columns:
    merged_df["company_name"] = merged_df["company_name_y"].fillna(
        merged_df["company_name_x"]
    )
...
merged_df["capital_allocation_pattern"] = merged_df[
...
st.info(
    f"Analyzing {len(merged_df)} companies across {len(pattern_counts)} capital allocation patterns"
)
...
if selected_pattern:
    pattern_companies = merged_df[
        merged_df["capital_allocation_pattern"] == selected_pattern
    ]

    if not pattern_companies.empty:
        st.info(
            f"{len(pattern_companies)} companies classified under '{selected_pattern}'"
        )
        ...
        for col in available_cols:
            if col not in ["company_id", "company_name", "broad_sector"]:
                results_df[col] = pd.to_numeric(results_df[col], errors="coerce").round(
                    2
                )
