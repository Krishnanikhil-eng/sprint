...
def parse_metric_text(text_val: Any) -> Tuple[Optional[int], Optional[float], Optional[str]]:
    """
...
def validate_cagr_against_db(
    ...
    if metric_type == "compounded_sales_growth":
        ...
        if not df_pnl.empty and len(df_pnl) >= 2:
            time_series = dict(zip(df_pnl['year'].astype(int), df_pnl['sales'].astype(float)))
            end_year = max(time_series.keys())
            computed_value, _ = calculate_series_cagr(time_series, end_year, period_years)
        ...
        if not df_pnl.empty and len(df_pnl) >= 2:
            time_series = dict(zip(df_pnl['year'].astype(int), df_pnl['net_profit'].astype(float)))
            end_year = max(time_series.keys())
            computed_value, _ = calculate_series_cagr(time_series, end_year, period_years)
...
def process_analysis_file(
    ...
    df_parsed = pd.DataFrame(parsed_records, columns=["company_id", "metric_type", "period_years", "value_pct"])
    df_failures = pd.DataFrame(failure_records, columns=["company_id", "metric_type", "original_value", "failure_reason"])
    df_validation = pd.DataFrame(validation_records, columns=[
        "company_id", "metric_type", "period_years", "parsed_value", "computed_value", "divergence_pct", "review_flag"
    ])
