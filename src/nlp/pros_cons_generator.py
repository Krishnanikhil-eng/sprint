...
class ProsConsEngine:
    ...
    def _load_data(self):
        ...
        if "broad_sector" in self.sectors.columns:
            fin_rows = self.sectors[
                self.sectors["broad_sector"].str.contains(
    ...
    def evaluate_company(self, cid: str) -> List[Dict[str, Any]]:
        ...
        is_debt_free = (latest_de is not None and latest_de <= 0.05) or (
            latest_borrowings == 0
        )
        ...
        if len(high_roe_yrs) >= 3 or (
            len(recent_roe) >= 2 and all(v > 20.0 for v in recent_roe)
        ):
        ...
        if (
            len(fcf_vals_5y) >= 4
            and sum(1 for v in fcf_vals_5y if v is not None and v > 0) >= 4
        ):
            conf = (
                85.0
                if len(fcf_vals_5y) == 5 and all(v > 0 for v in fcf_vals_5y)
                else 75.0
            )
            results.append(
                {
        ...
        avg_icr_3y = (
            np.mean([icr_ts.get(y) for y in years[-3:] if icr_ts.get(y) is not None])
            if len(years) >= 3
            else None
        )
        max_icr = max(
            [icr_ts.get(y) for y in years if icr_ts.get(y) is not None], default=0
        )
        ...
        if (
            is_debt_free
            or is_low_debt
            or (latest_icr is not None and latest_icr > 8.0)
            or (avg_icr_3y is not None and avg_icr_3y > 8.0)
            or max_icr > 10.0
        ):
        ...
        if (
            latest_div_yld is not None
            and latest_div_yld > 2.0
        ...
        if (
            rev_cagr_5y is not None
            and pat_cagr_5y is not None
            and rev_cagr_5y > 0
            and pat_cagr_5y > rev_cagr_5y
        ):
        ...
        if len(fcf_recent_3y) >= 2 and all(
            v is not None and v < 0 for v in fcf_recent_3y
        ):
        ...
        if len(years) >= 3:
            ...
            if (len(opm_seq) >= 2 and opm_seq[-1] < opm_seq[0] - 2.0) or (
                len(opm_seq) == 3 and opm_seq[0] > opm_seq[1] > opm_seq[2]
            ):
                results.append(
                    {
                        "company_id": cid,
        ...
        if len(years) >= 3:
            rev_seq = [
                sales_ts.get(y) for y in years[-3:] if sales_ts.get(y) is not None
            ]
            if (len(rev_seq) >= 2 and rev_seq[-1] < rev_seq[-2]) or (
                len(rev_seq) == 3 and rev_seq[0] > rev_seq[1] > rev_seq[2]
            ):
                results.append(
                    {
                        "company_id": cid,
    ...
    def generate_all(self, output_path: str = OUTPUT_CSV_DEFAULT) -> pd.DataFrame:
        ...
        companies_with_pro = df_filtered[df_filtered["type"] == "pro"][
            "company_id"
        ].nunique()
        companies_with_con = df_filtered[df_filtered["type"] == "con"][
            "company_id"
        ].nunique()
        companies_both = len(
            set(df_filtered[df_filtered["type"] == "pro"]["company_id"]).intersection(
                set(df_filtered[df_filtered["type"] == "con"]["company_id"])
            )
        )

        print("=== NLP Pros/Cons Generation Summary ===")
...
def run_pros_cons_generator(
    db_path: str = DB_PATH_DEFAULT, output_path: str = OUTPUT_CSV_DEFAULT
) -> pd.DataFrame:
