...
def compute_trend_arrow(
    curr: Optional[float], prev: Optional[float], lower_is_better: bool = False
) -> str:
    """
...
class PortfolioReportGenerator:
    def __init__(
        self, db_path: str = DB_PATH_DEFAULT, output_pdf: str = OUTPUT_PDF_DEFAULT
    ):
    ...
    def _load_data(self):
        ...
        self.companies = pd.read_sql_query(
            "SELECT * FROM companies ORDER BY company_id ASC", self.conn
        )
    ...
    def build_pdf(self) -> str:
        ...
        arrow_flat_style = ParagraphStyle(
            ...
            fontSize=11,
            leading=13,
            textColor=colors.HexColor("#E9C46A"),
        )
        ...
        for idx, cid in enumerate(comp_ids):
            ...
            opm_curr = l_rat.get("operating_profit_margin_pct") or l_pnl.get(
                "opm_percentage"
            )
            opm_prev = p_rat.get("operating_profit_margin_pct") or p_pnl.get(
                "opm_percentage"
            )
            ...
            kpi_data = [
                [
                    [
                        Paragraph("REVENUE", kpi_title_style),
                        Paragraph(
                            f"₹{sales_curr:,.0f} Cr" if sales_curr else "N/A",
                            kpi_val_style,
                        ),
                        Paragraph(f"YoY: {sales_arr}", get_arr_style(sales_arr)),
                    ],
                    [
                        Paragraph("NET PROFIT", kpi_title_style),
                        Paragraph(
                            f"₹{pat_curr:,.0f} Cr" if pat_curr else "N/A", kpi_val_style
                        ),
                        Paragraph(f"YoY: {pat_arr}", get_arr_style(pat_arr)),
                    ],
                    [
                        Paragraph("ROE", kpi_title_style),
                        Paragraph(
                            f"{roe_curr:.1f}%" if roe_curr is not None else "N/A",
                            kpi_val_style,
                        ),
                        Paragraph(f"YoY: {roe_arr}", get_arr_style(roe_arr)),
                    ],
                ],
                [
                    [
                        Paragraph("ROCE", kpi_title_style),
                        Paragraph(
                            f"{roce_curr:.1f}%" if roce_curr is not None else "N/A",
                            kpi_val_style,
                        ),
                        Paragraph(f"YoY: {roce_arr}", get_arr_style(roce_arr)),
                    ],
                    [
                        Paragraph("DEBT / EQUITY", kpi_title_style),
                        Paragraph(
                            f"{de_curr:.2f}x" if de_curr is not None else "N/A",
                            kpi_val_style,
                        ),
                        Paragraph(f"YoY: {de_arr}", get_arr_style(de_arr)),
                    ],
                    [
                        Paragraph("OPERATING MARGIN", kpi_title_style),
                        Paragraph(
                            f"{opm_curr:.1f}%" if opm_curr is not None else "N/A",
                            kpi_val_style,
                        ),
                        Paragraph(f"YoY: {opm_arr}", get_arr_style(opm_arr)),
                    ],
                ],
            ]
            ...
            story.append(
                Paragraph(
                    "<b>5-YEAR FINANCIAL HISTORICAL PERFORMANCE</b>",
                    ParagraphStyle(
                        "HistHeader",
                        parent=styles["Normal"],
                        fontName="Helvetica-Bold",
            ...
            pros_str = (
                ...
                else "Strong industry fundamentals."
            )
            cons_str = (
                pc_row["cons"].iloc[0]
                if not pc_row.empty and pd.notna(pc_row["cons"].iloc[0])
                else "Subject to sector trends."
            )
            ...
            footer_badge_data = [
                [
                    ...
                    Paragraph(
                        f"<b>Capital Allocation Pattern:</b> {cap_pattern.upper()}",
                        ParagraphStyle(
                            "FBadge",
                            parent=styles["Normal"],
                            fontName="Helvetica-Bold",
        ...
        doc.build(story)
        print(
            f"Generated Portfolio Summary PDF ({len(comp_ids)} pages) -> {self.output_pdf}"
        )
        return self.output_pdf


def run_portfolio_report(
    db_path: str = DB_PATH_DEFAULT, output_pdf: str = OUTPUT_PDF_DEFAULT
) -> str:
