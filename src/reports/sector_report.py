...
class SectorReportGenerator:
    ...
    def _get_sector_summary_df(self, sector_name: str) -> pd.DataFrame:
        ...
        sec_comps = self.sectors[self.sectors['broad_sector'] == sector_name]['company_id'].unique()
        if len(sec_comps) == 0:
            sec_comps = self.sectors[self.sectors['sub_sector'] == sector_name]['company_id'].unique()

        records = []
        for cid in sec_comps:
            c_info = self.companies[self.companies['company_id'] == cid]
            c_name = c_info['company_name'].iloc[0].split('\n')[0].strip() if not c_info.empty else cid
            ...
            records.append({
                ...
                "opm": l_rat.get('operating_profit_margin_pct') or l_pnl.get('opm_percentage'),
                "fcf": l_rat.get('free_cash_flow_cr')
            })

        return pd.DataFrame(records)

    def _generate_sector_chart(self, df_sec: pd.DataFrame, sector_name: str, chart_dir: str) -> str:
        ...
        ax1.set_title('Revenue & PAT Comparison', fontsize=8, fontweight='bold', color='#1A2B4C')
        ...
        ax2.set_title('Return Metrics (ROE vs ROCE)', fontsize=8, fontweight='bold', color='#1A2B4C')
    ...
    def build_sector_report(self, sector_name: str, output_path: Optional[str] = None) -> str:
        ...
        title_style = ParagraphStyle('SecTitle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=14, leading=16, textColor=colors.white)
        sub_style = ParagraphStyle('SecSub', parent=styles['Normal'], fontName='Helvetica', fontSize=9, leading=11, textColor=colors.HexColor('#D1E8FF'))
        ...
        # 1. Header Banner
        header_table = Table([[
            Paragraph(f"<b>SECTOR ANALYSIS REPORT: {sector_name.upper()}</b>", title_style),
        ...
        med_data = [
            [
                Paragraph("<b>MEDIAN SALES</b>", td_bold_style), Paragraph(f"₹{med_sales:,.0f} Cr" if pd.notna(med_sales) else "N/A", td_style),
                Paragraph("<b>MEDIAN PAT</b>", td_bold_style), Paragraph(f"₹{med_pat:,.0f} Cr" if pd.notna(med_pat) else "N/A", td_style),
                Paragraph("<b>MEDIAN ROE</b>", td_bold_style), Paragraph(f"{med_roe:.1f}%" if pd.notna(med_roe) else "N/A", td_style),
            ],
            [
                Paragraph("<b>MEDIAN ROCE</b>", td_bold_style), Paragraph(f"{med_roce:.1f}%" if pd.notna(med_roce) else "N/A", td_style),
                Paragraph("<b>MEDIAN D/E</b>", td_bold_style), Paragraph(f"{med_de:.2f}x" if pd.notna(med_de) else "N/A", td_style),
                Paragraph("<b>MEDIAN OPM</b>", td_bold_style), Paragraph(f"{med_opm:.1f}%" if pd.notna(med_opm) else "N/A", td_style),
            ]
        ]
        ...
        for _, row in df_sec.iterrows():
            table_rows.append([
                ...
                Paragraph(f"{row['sales']:,.0f}" if pd.notna(row['sales']) else "-", td_style),
                Paragraph(f"{row['net_profit']:,.0f}" if pd.notna(row['net_profit']) else "-", td_style),
                Paragraph(f"{row['roe']:.1f}" if pd.notna(row['roe']) else "-", td_style),
                Paragraph(f"{row['roce']:.1f}" if pd.notna(row['roce']) else "-", td_style),
                Paragraph(f"{row['de']:.2f}" if pd.notna(row['de']) else "-", td_style),
                Paragraph(f"{row['opm']:.1f}" if pd.notna(row['opm']) else "-", td_style),
                Paragraph(f"{row['fcf']:,.0f}" if pd.notna(row['fcf']) else "-", td_style),
            ])
...
def run_sector_reports(db_path: str = DB_PATH_DEFAULT, output_dir: str = OUTPUT_DIR_DEFAULT) -> List[str]:
