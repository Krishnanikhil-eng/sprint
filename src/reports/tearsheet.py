...
class TearsheetGenerator:
    def __init__(self, db_path: str = DB_PATH_DEFAULT, output_dir: str = OUTPUT_DIR_DEFAULT):
    ...
    def _generate_charts(self, cid: str, chart_dir: str) -> Tuple[str, str, str, str]:
        ...
        rects1 = ax1.bar(x - width/2, sales, width, label='Revenue (₹ Cr)', color='#1A2B4C')
        rects2 = ax1.bar(x + width/2, pat, width, label='Net Profit (₹ Cr)', color='#008080')
        ...
        ax1.set_title('10-Year Revenue & Net Profit Growth', fontsize=9, fontweight='bold', color='#1A2B4C', pad=4)
        ...
        ax2.plot(r_years, roe, marker='o', linewidth=2, label='ROE (%)', color='#E76F51')
        ax2.plot(r_years, roce, marker='s', linewidth=2, linestyle='--', label='ROCE (%)', color='#2A9D8F')
        ...
        ax2.set_title('Return Metrics Trend (ROE vs ROCE)', fontsize=9, fontweight='bold', color='#1A2B4C', pad=4)
        ...
        if not c_bs.empty:
            ...
            ax3.bar(b_years, other_liab, bottom=equity+borrowings, label='Other Liab', color='#E9C46A')

            ax3.set_title('Balance Sheet Composition', fontsize=8, fontweight='bold', color='#1A2B4C')
        ...
        if not c_cf.empty:
            ...
            ax4.set_title('Cash Flow Breakdown (Latest)', fontsize=8, fontweight='bold', color='#1A2B4C')
    ...
    def build_pdf(self, cid: str, output_path: Optional[str] = None) -> str:
        ...
        with tempfile.TemporaryDirectory() as tmp_chart_dir:
            ...
            # Custom Styles
            title_style = ParagraphStyle('HeaderTitle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=14, leading=16, textColor=colors.white)
            sub_title_style = ParagraphStyle('HeaderSub', parent=styles['Normal'], fontName='Helvetica', fontSize=9, leading=11, textColor=colors.HexColor('#D1E8FF'))

            tile_label_style = ParagraphStyle('TileLabel', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=7, leading=9, textColor=colors.HexColor('#555555'))
            tile_val_style = ParagraphStyle('TileVal', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=10, leading=12, textColor=colors.HexColor('#1A2B4C'))
            tile_sub_style = ParagraphStyle('TileSub', parent=styles['Normal'], fontName='Helvetica', fontSize=7, leading=9, textColor=colors.HexColor('#2A9D8F'))

            sec_header_style = ParagraphStyle('SecHeader', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=10, leading=12, textColor=colors.HexColor('#1A2B4C'))
            bullet_style = ParagraphStyle('BulletText', parent=styles['Normal'], fontName='Helvetica', fontSize=8, leading=10, textColor=colors.HexColor('#222222'))
            ...
            # 1. Header Bar
            comp_name_clean = str(comp_info.get('company_name', cid)).split('\n')[0].strip()
            header_table_data = [
                [
                    Paragraph(f"<b>{comp_name_clean}</b> ({cid})", title_style),
                    Paragraph(f"<b>Sector:</b> {broad_sec} | {sub_sec}", sub_title_style)
                ]
            ]
            ...
            rev_val = f"₹{l_pnl.get('sales', 0):,.0f} Cr" if l_pnl.get('sales') else "N/A"
            rev_cagr = f"5Y CAGR: {l_rat.get('revenue_cagr_5yr', 0):.1f}%" if l_rat.get('revenue_cagr_5yr') is not None else "5Y CAGR: N/A"

            pat_val = f"₹{l_pnl.get('net_profit', 0):,.0f} Cr" if l_pnl.get('net_profit') else "N/A"
            pat_cagr = f"5Y CAGR: {l_rat.get('pat_cagr_5yr', 0):.1f}%" if l_rat.get('pat_cagr_5yr') is not None else "5Y CAGR: N/A"

            roe_val = f"{l_rat.get('return_on_equity_pct', 0):.1f}%" if l_rat.get('return_on_equity_pct') is not None else "N/A"
            roce_val = f"ROCE: {l_rat.get('roce_pct', 0):.1f}%" if l_rat.get('roce_pct') is not None else "ROCE: N/A"

            de_val = f"{l_rat.get('debt_to_equity', 0):.2f}x" if l_rat.get('debt_to_equity') is not None else "N/A"
            icr_val = f"ICR: {l_rat.get('interest_coverage', 0):.1f}x" if l_rat.get('interest_coverage') is not None else "ICR: N/A"

            cfo_qual = str(l_rat.get('cfo_quality_label', 'N/A'))
            fcf_val = f"FCF: ₹{l_rat.get('free_cash_flow_cr', 0):,.0f} Cr" if l_rat.get('free_cash_flow_cr') is not None else "FCF: N/A"

            mcap_val = f"₹{l_mkt.get('market_cap_crore', 0):,.0f} Cr" if l_mkt.get('market_cap_crore') else "N/A"
            pe_val = f"P/E: {l_mkt.get('pe_ratio', 0):.1f}x" if l_mkt.get('pe_ratio') is not None else "P/E: N/A"

            kpi_data = [
                [
                    [Paragraph("REVENUE", tile_label_style), Paragraph(rev_val, tile_val_style), Paragraph(rev_cagr, tile_sub_style)],
                    [Paragraph("NET PROFIT", tile_label_style), Paragraph(pat_val, tile_val_style), Paragraph(pat_cagr, tile_sub_style)],
                    [Paragraph("RETURN ON EQUITY", tile_label_style), Paragraph(roe_val, tile_val_style), Paragraph(roce_val, tile_sub_style)],
                ],
                [
                    [Paragraph("LEVERAGE (D/E)", tile_label_style), Paragraph(de_val, tile_val_style), Paragraph(icr_val, tile_sub_style)],
                    [Paragraph("CFO QUALITY", tile_label_style), Paragraph(cfo_qual, tile_val_style), Paragraph(fcf_val, tile_sub_style)],
                    [Paragraph("MARKET CAP", tile_label_style), Paragraph(mcap_val, tile_val_style), Paragraph(pe_val, tile_sub_style)],
                ]
            ]
            ...
            p2_header_data = [
                [
                    Paragraph(f"<b>{comp_name_clean}</b> — Balance Sheet, Cash Flow & Qualitative Signals", title_style)
                ]
            ]
            ...
            charts_table_data = [
                [Image(path3, width=265, height=165), Image(path4, width=265, height=165)]
            ]
            ...
            pros_text = pc_row['pros'].iloc[0] if not pc_row.empty and pd.notna(pc_row['pros'].iloc[0]) else "Consistent operational execution."
            cons_text = pc_row['cons'].iloc[0] if not pc_row.empty and pd.notna(pc_row['cons'].iloc[0]) else "Subject to general macroeconomic risks."
            ...
            pros_flowables = [Paragraph("<b>KEY STRENGTHS & INVESTMENT PROS</b>", ParagraphStyle('ProTitle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=8, leading=10, textColor=colors.HexColor('#2A9D8F')))]
            for p in pros_bullets[:4]:
                pros_flowables.append(Paragraph(f"• {p}", bullet_style))

            cons_flowables = [Paragraph("<b>KEY RISKS & FINANCIAL CONCERNS</b>", ParagraphStyle('ConTitle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=8, leading=10, textColor=colors.HexColor('#E63946')))]
            for c in cons_bullets[:4]:
                cons_flowables.append(Paragraph(f"• {c}", bullet_style))

            pc_table_data = [
                [pros_flowables, cons_flowables]
            ]
            ...
            badge_color = colors.HexColor('#2A9D8F') if cap_pattern in ['Shareholder Returns', 'Reinvestor', 'Cash Accumulator'] else colors.HexColor('#E63946')

            badge_title_style = ParagraphStyle('BadgeTitle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=8, leading=10, textColor=colors.HexColor('#555555'))
            badge_val_style = ParagraphStyle('BadgeVal', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=12, leading=14, textColor=badge_color)

            badge_data = [
                [
                    Paragraph("CAPITAL ALLOCATION PATTERN (SPRINT 2 ENGINE)", badge_title_style),
                    Paragraph(f"CLASSIFICATION BADGE: <b>{cap_pattern.upper()}</b>", badge_val_style)
                ]
            ]
...
def run_batch_test_tearsheets(db_path: str = DB_PATH_DEFAULT, output_dir: str = "reports/test_tearsheets") -> List[str]:
