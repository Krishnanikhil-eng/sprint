...
from src.analytics.cluster_profiler import ClusterProfiler, run_day37_analytics, PORTFOLIO_10_KPIS
...
def test_cluster_profiler_outputs(tmp_path):
    ...
    profiler = ClusterProfiler(db_path="nifty100.db", labels_csv="output/cluster_labels.csv")
    ...
    assert set(df_outliers.columns).issuperset({"company_id", "broad_sector", "metric", "z_score"})
    ...
    assert set(df_stats.columns).issuperset({"metric", "count", "mean", "std", "p10", "p50", "p90"})
