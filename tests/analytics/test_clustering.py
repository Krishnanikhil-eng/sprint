...
from src.analytics.clustering import FinancialClusteringEngine, run_clustering, CLUSTERING_FEATURES
...
def test_prepare_features_imputation(tmp_path):
    ...
    cur.execute("CREATE TABLE financial_ratios (company_id TEXT, year INT, return_on_equity_pct REAL, roce_pct REAL, debt_to_equity REAL, interest_coverage REAL, revenue_cagr_5yr REAL, fcf_cagr_5yr REAL, operating_profit_margin_pct REAL);")
    ...
    cur.execute("INSERT INTO financial_ratios VALUES ('C1', 2024, 25.0, 20.0, 0.1, 15.0, 15.0, 10.0, 20.0);")
    ...
    cur.execute("INSERT INTO financial_ratios VALUES ('C2', 2024, NULL, 16.0, 0.2, 10.0, 12.0, NULL, 18.0);")
    ...
    assert df_imp.loc[df_imp['company_id'] == 'C2', 'return_on_equity_pct'].iloc[0] == 25.0 # Imputed from C1 sector median
...
def test_kmeans_clustering_and_elbow_plot(tmp_path):
    ...
    df_labels = run_clustering(db_path="nifty100.db", output_csv=output_csv, elbow_path=elbow_path)
