...
def test_distress_and_deleveraging_logic(tmp_path):
    ...
    cur.execute(
        "CREATE TABLE profitandloss (company_id TEXT, year INT, sales REAL, net_profit REAL, operating_profit REAL);"
    )
    cur.execute(
        "CREATE TABLE balancesheet (company_id TEXT, year INT, borrowings REAL);"
    )
    cur.execute(
    ...
    cur.execute(
        "INSERT INTO profitandloss VALUES ('DISTRESS_CO', 2024, 100.0, -10.0, 15.0);"
    )
    ...
    cur.execute(
        "INSERT INTO profitandloss VALUES ('DELEVERAGED_CO', 2023, 500.0, 100.0, 150.0);"
    )
    cur.execute(
        "INSERT INTO profitandloss VALUES ('DELEVERAGED_CO', 2024, 600.0, 120.0, 180.0);"
    )
    ...
    cur.execute(
        "INSERT INTO cashflow VALUES ('DELEVERAGED_CO', 2023, 150.0, -50.0, -80.0);"
    )
    cur.execute(
        "INSERT INTO cashflow VALUES ('DELEVERAGED_CO', 2024, 160.0, -60.0, -90.0);"
    )

    conn.commit()
    conn.close()

    df_res, df_distress = run_cashflow_intelligence(
        db_path=db_file, output_excel=excel_file, alerts_csv=alerts_file
    )
