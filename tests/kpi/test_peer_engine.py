...
def test_inverse_debt_percentile_logic(peer_engine):
    ...
    if len(grp) > 0:
        sample_grp = grp.groupby("effective_peer_group").get_group(grp["effective_peer_group"].iloc[0])
        ...
        assert lowest_de["debt_to_equity_percentile"] >= highest_de["debt_to_equity_percentile"]
