...
def test_screener_config_evaluation():
    crit = FilterCriterion("return_on_equity_pct", FilterOperator.GREATER_THAN, value=15.0)
...
def test_financials_de_exemption():
    ...
    fin_companies = df_raw[df_raw["broad_sector"].str.contains("Financial", case=False, na=False)]
    if len(fin_companies) > 0:
        ...
        if len(high_de_fin) > 0:
            config = ScreenerConfig(
                ...
                criteria=[FilterCriterion("debt_to_equity", FilterOperator.LESS_EQUAL, value=1.0)],
                handle_financials_de=True
            )
            engine.set_config(config)
            filtered = engine.apply_filters()
            fin_passed = filtered[filtered["broad_sector"].str.contains("Financial", case=False, na=False)]
...
def test_zero_debt_icr_handling():
    ...
    config = ScreenerConfig(
        ...
        criteria=[FilterCriterion("interest_coverage", FilterOperator.GREATER_EQUAL, value=5.0)],
