...
def mock_engine(tmp_path):
    ...
    cur.execute(
        """CREATE TABLE balancesheet (company_id TEXT, year INT, borrowings REAL, total_assets REAL, investments REAL);"""
    )
    cur.execute(
        """CREATE TABLE cashflow (company_id TEXT, year INT, operating_activity REAL, investing_activity REAL, financing_activity REAL);"""
    )
    cur.execute(
        """CREATE TABLE financial_ratios (company_id TEXT, year INT, return_on_equity_pct REAL, roce_pct REAL, debt_to_equity REAL, interest_coverage REAL);"""
    )
    cur.execute(
        """CREATE TABLE market_cap (company_id TEXT, year INT, dividend_yield_pct REAL);"""
    )
...
def test_pro_1_roe_sustained(mock_engine):
    ...
    for y in [2022, 2023, 2024]:
        cur.execute(
            "INSERT INTO financial_ratios (company_id, year, return_on_equity_pct) VALUES ('TEST1', ?, 22.0)",
            (y,),
        )
        cur.execute(
            "INSERT INTO profitandloss (company_id, year) VALUES ('TEST1', ?)", (y,)
        )
...
def test_pro_3_debt_free(mock_engine):
    ...
    cur.execute(
        "INSERT INTO financial_ratios (company_id, year, debt_to_equity) VALUES ('TEST3', 2024, 0.0)"
    )
    mock_engine.conn.commit()
    mock_engine._load_data()
...
def test_con_1_high_de(mock_engine):
    ...
    cur.execute(
        "INSERT INTO profitandloss (company_id, year) VALUES ('TEST_LEVERAGED', 2024)"
    )
    cur.execute(
        "INSERT INTO financial_ratios (company_id, year, debt_to_equity) VALUES ('TEST_LEVERAGED', 2024, 2.5)"
    )
    mock_engine.conn.commit()
    mock_engine._load_data()
...
def test_con_4_net_loss(mock_engine):
    ...
    cur.execute(
        "INSERT INTO profitandloss (company_id, year, net_profit) VALUES ('TEST_LOSS', 2024, -50.0)"
    )
    mock_engine.conn.commit()
    mock_engine._load_data()
