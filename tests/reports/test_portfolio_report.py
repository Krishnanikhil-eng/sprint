...
def test_trend_arrow_calculations():
    ...
    assert (
        compute_trend_arrow(0.5, 0.8, lower_is_better=True) == "↑"
    )  # Improved (lower debt)
    assert (
        compute_trend_arrow(0.8, 0.5, lower_is_better=True) == "↓"
    )  # Declined (higher debt)
    assert (
        compute_trend_arrow(0.50, 0.51, lower_is_better=True) == "→"
    )  # Flat within 2%
...
def test_portfolio_report_generation(tmp_path):
    ...
    cur.execute(
        "CREATE TABLE financial_ratios (company_id TEXT, year INT, return_on_equity_pct REAL, roce_pct REAL, debt_to_equity REAL, interest_coverage REAL, operating_profit_margin_pct REAL, free_cash_flow_cr REAL, cfo_quality_label TEXT, capital_allocation_pattern TEXT);"
    )
    cur.execute(
    ...
    for cid in ["TCS", "AAC", "MM"]:
        ...
        for y in [2023, 2024]:
            cur.execute(
                "INSERT INTO profitandloss VALUES (?, ?, 1000.0, 200.0, 20.0);",
                (cid, y),
            )
            cur.execute(
                "INSERT INTO financial_ratios VALUES (?, ?, 20.0, 18.0, 0.1, 15.0, 20.0, 100.0, 'High Quality', 'Reinvestor');",
                (cid, y),
            )
            cur.execute(
                "INSERT INTO market_cap VALUES (?, ?, 10000.0, 25.0);", (cid, y)
            )
            cur.execute(
                "INSERT INTO prosandcons VALUES (?, 'Good ROE', 'High P/E');", (cid,)
            )
