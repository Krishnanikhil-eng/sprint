...
def test_generate_5_test_tearsheets(tmp_path):
    ...
    for pdf_path in files:
        ...
        assert (
            size_bytes >= 30 * 1024
        ), f"File {pdf_path} size is too small: {size_bytes} bytes"

        # Check page count is strictly 2 pages
        page_count = get_pdf_page_count(pdf_path)
        assert (
            page_count == 2
        ), f"PDF {pdf_path} has {page_count} pages, expected exactly 2"
...
def test_tearsheet_layout_edge_cases(tmp_path):
    ...
    cur.execute(
        "INSERT INTO sectors VALUES ('LONG_CO', 'Conglomerates & Diversified Industrial Capital Goods', 'Heavy Industrial Machinery');"
    )
    cur.execute(
        "INSERT INTO profitandloss VALUES ('LONG_CO', 2024, 150000.0, 25000.0, 22.0, 150.0, 30.0, 33000.0, 2000.0);"
    )
    cur.execute(
        "INSERT INTO balancesheet VALUES ('LONG_CO', 2024, 5000.0, 95000.0, 10000.0, 120000.0);"
    )
    cur.execute(
        "INSERT INTO cashflow VALUES ('LONG_CO', 2024, 30000.0, -15000.0, -10000.0, 5000.0);"
    )
    cur.execute(
        "INSERT INTO financial_ratios VALUES ('LONG_CO', 2024, 25.0, 22.0, 0.10, 15.0, 18.0, 22.0, 'High Quality', 15000.0, 'Reinvestor');"
    )
    cur.execute("INSERT INTO market_cap VALUES ('LONG_CO', 2024, 500000.0, 20.0);")
    cur.execute(
        "INSERT INTO prosandcons VALUES ('LONG_CO', ?, ?);", (long_pro, long_con)
    )
